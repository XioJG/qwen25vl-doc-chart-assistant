...
本项目面向简历展示、面试演示和后续持续迭代，目标是搭建一个可在本地运行的多模态文档/图表理解系统，而不是只调用远程 API。项目重点体现：

- 本地部署 Qwen2.5-VL-3B-Instruct。
- 处理文档、图表、网页界面截图等视觉输入。
- 通过任务 prompt 实现问答、字段抽取、UI 理解和 JSON 输出。
- 提供 CLI、批量推理、基础评测和 Gradio Demo。
- 为后续 LoRA/QLoRA 微调、基线对比和面试展示保留清晰工程结构。
...
- **工程化 Demo**：同时提供命令行推理、批量推理和 Gradio Web Demo，方便调试与展示。
- **可扩展训练路线**：保留 LoRA/QLoRA 微调脚本说明，但明确标注为后续计划，不虚构指标或训练效果。

## 面试展示导航

如果只用 30 秒浏览本仓库，建议按这个顺序看：
...
4. 最后看 [docs/screenshots](docs/screenshots) 中后续补充的脱敏 Demo 截图，用于展示 CLI / Gradio / JSON 输出效果。
...
> 基于 Qwen2.5-VL-3B-Instruct 搭建本地多模态文档/图表理解与结构化抽取系统，支持文档截图、图表页面和网页界面截图的图像问答、字段抽取和 JSON 输出；完成 CLI 推理、Gradio Demo、数据格式转换、批量推理与基础评测脚本，并规划 LoRA/QLoRA 微调和微调前后对比实验。

当前不建议写成“已完成微调并显著提升指标”，因为 LoRA/QLoRA 结果仍处于后续计划阶段。
